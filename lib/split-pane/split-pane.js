...
//		$splitPanes.children('.split-pane-divider').html('<div class="split-pane-divider-inner"></div>');
		$splitPanes.children('.split-pane-divider').on('mousedown', mousedownHandler);
		setTimeout(function() {
	...
	function getComponentsSizes($splitPane) {
		var property = $splitPane.is('.fixed-top, .fixed-bottom, .horizontal-percent') ?
			'height' : 'width';
		return {
			firstComponentSize: $splitPane.children('.split-pane-component:first')[property](),
			lastComponentSize: $splitPane.children('.split-pane-component:last')[property]()
		};
	}
