...
//		this.html = "./ShiXu.html";
		this.css = ["./ami.css",'./MyTag.css'];
		this.script =['./MyBubble.js', './Track.js'];
	}
  	...
  	TargetSearchResult(para)
  	{
  		console.log('zfc test')
  		console.log(para)
	  }
	
	_targetSearchResult(info) {
		if(!this.earth.TargetSearchResult) return;
		this.earth.TargetSearchResult(info);
		this.dynTarget.off('dyntarget_search', this._targetSearchResult);
		this.dynTarget.on('dyntarget_search', this.earth.TargetSearchResult);
	}
  	
  	_tempFun(info) {	
  		if(!this.earth.tempFun) return;
  		console.log('111');
  		this.earth.tempFun(info);
  		this.dynTarget.off('dyntarget_create',this._tempFun);
  		this.dynTarget.on('dyntarget_create',this.earth.tempFun);
  	}
    ...
    init() {
        ...
        this.dynTarget.on('dyntarget_dhfk_warn', this.DhfkWarn);
      	this.dynTarget.on('dyntarget_count',this.TargetNumber);
        this.dynTarget.on('dyntarget_create',this._tempFun.bind(this));
        ...
        this.dynTarget.on('dyntarget_search',this._targetSearchResult.bind(this));
        ...
        this.trackScene = new GV.GraphicScene(); //创建专题场景节点
        this.earth.addScene(this.trackScene); //将专题场景节点添加到地球
        
        
        //初始化航迹点信息显示标牌
        this.trackInfoBubble = new GV.Bubble({
            // bindingObject: place2.id,
            offset: {
                x: 125,
                y: -25
            },
            anchorType: GV.BUBBLETYPE.GEO_BUBBLE
        });
        this.trackInfoBubble.lineDiv[0].style.borderColor = '#cccccc';
        this.trackInfoBubble.moveDiv.html(`<div class="trackPointInfoBubble" style="height: 150px; width: 150px; background: #eeeeee">
            <ul>
                <li>
                    <span>id:</span>
                    <span id="trackPointPHId"></span>
                </li>
                <li>
                    <span>LBDW:</span>
                    <span id="trackPointPHLBDW"></span>
                </li>
                <li>
                    <span>XTMBBH:</span>
                    <span id="trackPointPHXTMBBH"></span>
                </li>
                <li>
                    <span>MBSL:</span>
                    <span id="trackPointPHLBDW"></span>
                ...
                <li>
                    <span>PH:</span>
                    <span id="trackPointLBDW"></span>
                </li>
            </ul>
        </div>`);

        this.trackInfoBubble.userVisible = false;   // 初始状态为隐藏
        this.trackScene.addNode(this.trackInfoBubble);
...
//  	console.log(info)
        ...
        {
        	var strAudio = "<audio id='audioPlay' src='' hidden='true'>";
	...
	SaveCurrentArea(areaName) { this.dynTarget.postMessage('SaveCurrentArea',areaName); }
	RequestCurrentArea(areaName) {  this.dynTarget.postMessage('RequestCurrentArea',areaName); }
	LookatTarget(id) {  this.dynTarget.postMessage('LookatTarget',id); }
	...
	RouteType(v) { this.dynTarget.postMessage('RouteType',v); }
	...
	RadarUnionHeight(heiList){this.dynTarget.postMessage('RadarUnionHeight',heiList);}
	RadarUnionName(nameList){	this.dynTarget.postMessage('RadarUnionName',nameList);}
	createtargetType(v){ this.dynTarget.postMessage('createtargetType',v); }
	TargetWarn(id,v){this.dynTarget.postMessage('TargetWarn',id+";"+v);}
	SetRadarShowType(){this.dynTarget.postMessage('SetRadarShowType',"");}
	TargetThread(id,v){this.dynTarget.postMessage('TargetThread',id+";"+v);}
	JQType(v){this.dynTarget.postMessage('JQType',v);}
	TargetVisible(v){this.dynTarget.postMessage('TargetVisible',v);}
	RadarVisible(v){this.dynTarget.postMessage('TargetVisible',v);}
	searchTZ(v){this.dynTarget.postMessage('SearchTZ',v);}
	searchTarget(v){this.dynTarget.postMessage('SearchTarget',v);}
	...
	_updataInfo() {
		const test_data = {
			row__array__: [
				...
				{id: 'detail', longitude: 100.2, latitude: 20.0, altitude: 0, bjdm: '布吉岛', time: '2017-06-14 08:56:12', ssbd: 'A部队', dwsx: '敌', mmlb: 'kz目标'},
		...
		for (let i = 0; i < mubiao_list.length; i++) {
			...
			const info = {
				...
				showAttr: [
					...
					{
						type: 'func',
					    name: ['知识库', '重点目标', '雷达范围', '打击范围', '告警','威胁分析','轨迹'],
					    attr: [`zslib_func_${id}`, `yxtx_func_${id}`, `ldfw_func_${id}`, `djfw_func_${id}`, `gjfw_func_${id}`, `wxfw_func_${id}`, `gjhffw_func_${id}`]
					}
				],
				...
				if (!this._info.get(value.id)) this._info.set(value.id, {
					...
					showAttr: [
						...
						{type: 'half', name: ['部队', '敌我'], attr: ['ssbd', 'dwsx'], value: ["暂无", "暂无"]},
						{
							type: 'func',
							name: ['知识库', '重点目标', '雷达范围', '打击范围', '告警','威胁分析','轨迹'],
							attr: [`zslib_func_${value.id}`, `yxtx_func_${value.id}`, `ldfw_func_${value.id}`, `djfw_func_${value.id}`, `gjfw_func_${value.id}`, `wxfw_func_${value.id}`,`gjhffw_func_${value.id}`]
						}
					],
					otherAttr: {}
				});
		...
		if (info) {
			info.showAttr[info.showAttr.length - 1].attr.forEach((value, index) => {
				if (index === 3) { // 定义clickFn 和 closeFn的情况
					...
					bubble.clickFn[index] = (id, info) => {
						this.TargetWarn(id,true);
						//alert('gaojing')
					};
					bubble.closeFn[index] = (id, info) => { 
						this.TargetWarn(id,false); 
					};
				} else if (index === 5) {
					bubble.clickFn[index] = (id, info) => {
						this.TargetThread(id,true); 
					};
					bubble.closeFn[index] = (id, info) => { 
						this.TargetThread(id,false); 
					};
				} else if (index === 6) {
					bubble.clickFn[index] = (id, info) => {
						//this.TargetThread(id,true); 
						...
						if(this.trackShow) {
							if(this.trackInfoBubble.userVisible) {
								this.trackInfoBubble.userVisible = false;
							}
							this.trackInfoBubble.bindingObject = '';
							this.track.removeTrack();
							this.track = null;
							this.trackShow = false;
						}
						
						this.drawTrack('10', this.trackScene);
						this.trackShow = true;
						// alert('gaojing')
					};
					bubble.closeFn[index] = (id, info) => { 
    ...
    drawTrack (id ,scene) {
    		let that = this;
            $.getJSON(id + '.json', function(track){
                that.track = new Track(scene, track.ShipInfo, track.TrackPoints, 2, '#FFFFAA');
            })

            this.earth.container.mousedown((event)=>{

                that.earth.pick((obj)=>{
                	
//              	if(!that.track) return;

                    // 没选中任何物体时
                    if(!obj) {
                        //隐藏bubble
                        if(that.trackInfoBubble.userVisible) {
                            that.trackInfoBubble.userVisible = false;
                            that.trackInfoBubble.bindingObject = '';
                        }
                        // 恢复选中航迹点样式
                        if( that.track && that.track.selectTrackPoint) {
                            that.track.resetSelectedPointStyle();
                        }
                        return;
                    }
					
                    // 更具ID获得对应航迹点的信息，不为航迹点则返回空
                    let pointInfo = that.track.getPointInfo(obj.id);
                    if(!pointInfo) {
                    	
                        if(that.trackInfoBubble.bindingObject) {
                            that.trackInfoBubble.userVisible = false;
                            that.trackInfoBubble.bindingObject = '';
                        }
                        if( that.track && that.track.selectTrackPoint) {
                    ...
                    // 当选中了航迹点，并且航迹点为已选中航迹点则直接返回
                    if(that.track.selectTrackPoint && that.track.selectTrackPoint.id === pointInfo.id) {
                        return;
                    }

                    // 恢复上次选中的航迹点图标和大小
                    if( that.track.selectTrackPoint) {
                        that.track.resetSelectedPointStyle();
                    }
                    
                    // 更新重新选中的航迹点
                    that.trackInfoBubble.bindingObject = obj.id;
                    obj.icon = 'http://127.0.0.1:3000/docResource/trackIcon_select.png';
                    obj.size = 12;
                    that.track.selectTrackPoint = obj;
                    that.trackInfoBubble.userVisible = true;
                    $('.info').show();
                    $('#trackPointId').text(pointInfo._id);
                    $('#trackPointLBDW').text(pointInfo.LBDW);
                    $('#trackPointXTMBBH').text(pointInfo.XTMBBH);
                    $('#trackPointMBSL').text(pointInfo.MBSL);
                    $('#trackPointPH').text(pointInfo.PH);
                });
            });
        }
}
