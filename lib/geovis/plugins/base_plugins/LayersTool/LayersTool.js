...
let LayersTool;
{
    ...
    LayersTool = (
        class Layers extends GV.Tool {
            ...
            reference() {
                this.css = './LayersTool.css';
                this.script = './Viewer.js';
            }

            init() {
                console.debug(this.container.id);
                const layers = this.earth.getLayers();
                const baseLayer = this.earth.getBaseLayer();
                this.initDOM({ layers, baseLayer });
            }
            ...
            initDOM(initLayers) {
                ...
                if (!this.dom) {
                    this.layerToolDom = new LayersToolViewer.LayersTool({
                        container: this.container.id,
                        url: this.formatURL('./layers.json'),
                        layers: {
                            images: inits,
                        ...
                        // undefined 说明取消添加基础图层，不做操作，还是原来的，暂且不做变白球的操作
                            ...
                            switch (item.type) {
                                ...
                                case 'vector': this.addVector(item); break;
                                default: break;
                            }
                        },
                ...
                if (item.type === 'image') {
                    switch (item.source.driver) {
                        ...
                        case 'xyz':
                            ...
                            break;
                        case 'wms':
                            source = new GV.WebMapServiceSource({
                                url: item.source.url,
                                format: item.source.format,
                                layers: item.source.layers,
                                tileSize: item.source.tileSize,
                                srs: item.source.srs
                            });
                            break;
                        case 'grst':
            ...
            addVector(item) {
                ...
                if (item.type === 'vector') {
                    switch (item.source.driver) {
                        case 'ogr':
                            source = new GV.OgrModelSource({
                                url: item.source.url,
                            });
                            break;
                        default: break;
                    }
                    this.earth.addVectorLayer({
                        name: item.id,
                        source,
                        styles: item.styles,
                        layout: item.layout
