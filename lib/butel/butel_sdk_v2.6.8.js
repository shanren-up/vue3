...
(function(window){
   ...
   /*var _config_param = {
    ...
    var _config_param = {
        "nube":"67003106",// nube just for development
     ...
     * @param { Function } errorCB
     */
    function _deleteMeeting(meetingId, successCB, errorCB){
        var paramObj = {
        ...
        _ajax({
            ...
            method:"post",
            success: function(data, resultText){ successCB?successCB(data, resultText): true},
            error: function(err, resultText) { errorCB?errorCB(err, resultText): true }
        });
    }
     ...
     * @param { Function } error
     * */
    function deleteCombatRoom(meetingId,success,error){
        var thiz = this;
        ...
        _wrapLog(thiz,"log","ButelSDK:COMBAT deleteCombatRoom,start,deleteMeeting request");
        
        //delete meeting
        _deleteMeeting(meetingId, function(data, resultText){
            _wrapLog(thiz,"log","ButelSDK:COMBAT deleteCombatRoom,start,deleteMeeting request successfully");
            _wrapSetAPIBusy(thiz,false,"deleteCombatRoom");

            if(data["result"]["rc"] === 0){
                _wrapLog(thiz,"log","ButelSDK:COMBAT deleteCombatRoom,done,deleteMeeting request successfully,invoke success cb");
                success(JSON.stringify({"code":0,"msg":"ok","data":null}),resultText);
            }else  if(data["result"]["rc"] === -999){
                _wrapLog(thiz,
                    "error",
                    "ButelSDK:COMBAT deleteCombatRoom,done,deleteMeeting request successfully,but service internal error",
                    JSON.stringify({"code":-999,"msg":"service internal error"}),"business error"
                );
                error(JSON.stringify({"code":-999,"msg":"service internal error"}),"business error");
            }else if(data["result"]["rc"] === -902){
                ...
                _wrapLog(thiz,
                    "error",
                    "ButelSDK:COMBAT deleteCombatRoom,done,deleteMeeting request successfully,but ticket invalid",
                    JSON.stringify({"code":-2,"msg":"ticket invalid"}),"business error"
                );
                error(JSON.stringify({"code":-2,"msg":"ticket invalid"}),"business error");
            }else{
                _wrapLog(thiz,
                    "error",
                    "ButelSDK:COMBAT deleteCombatRoom,done,deleteMeeting request successfully,but param error",
                    JSON.stringify({"code":-3,"msg":"param error"}),"business error"
                );
                error(JSON.stringify({"code":-3,"msg":"param error"}),"business error");
            }
            ...
            _wrapSetAPIBusy(thiz,false,"deleteCombatRoom");
            _wrapLog(thiz,"error","ButelSDK:COMBAT deleteCombatRoom,error,deleteMeeting request with error",JSON.stringify(err),resultText);
            error(JSON.stringify(err),resultText);
        }); 
    }
